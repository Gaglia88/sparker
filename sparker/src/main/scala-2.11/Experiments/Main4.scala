...
import Utilities.{Converters, StatisticsEstimator}
...
object Main4 {
  def main(args: Array[String]) {
    ...
    /*val edges = WNPCBSFor.WNPAdv(profileBlocksFiltered, blockIndex, maxProfileID.toInt, separatorID, WNPTypes.OR)
    ...
    println()
    println("Time to pruning edges "+(pruningTime.getTimeInMillis-blocksFilteringTime.getTimeInMillis)+" ms")
    println()*/
    ...
    val stats = StatisticsEstimator.estimatePCPQ(groundtruth, profiles, edges)
