...
val appender = new FileAppender(layout,"/marconi_scratch/userexternal/lgaglia1/log/logDbpediaProfilesGenerator.txt",false);
...
val data = sc.textFile(basePath+"dbpedia.nq")
...
val dbpediaIDs = sc.broadcast(gt.map(_.firstEntityID).collect().toSet)
val linesInTheGroundtruth = linesByID.filter(x => dbpediaIDs.value.contains(x._1))
...
profilesGtJSON.repartition(1).saveAsTextFile(basePath+"DBpediaProfilesGT")
linesInTheGroundtruth.map(_._1).repartition(1).saveAsTextFile(basePath+"DBpediaGtProfilesIDs")
...
profilesGtJSON.union(profilesNotGtJSON).repartition(1).saveAsTextFile(basePath+"AllDBpediaProfiles")
