...
object Main2 {
  def main(args: Array[String]) {
    ...
    val pathGt = "C:\\Users\\Luca\\Downloads\\citations\\citations\\matches_citeseer_dblp.csv"
    ...
    val groundtruth = loadGT(pathGt)

    groundtruth.filter(_.firstEntityID == "266073").foreach(println)

    /*groundtruth.take(10).foreach(println)

    val a = groundtruth.map(x => (x.firstEntityID, x.secondEntityID))
    val dup1 = a.groupByKey().filter(_._2.size > 1)
    val dup2 = a.map(_.swap).groupByKey().filter(_._2.size > 1)
    val dup1Num = dup1.count()
    val dup2Num = dup2.count()

    dup1.take(10).foreach(println)
    println()
    println()
    println(dup1Num)
    println(dup2Num)
*/
    ...
    val matchFound = loadGT("C:/Users/Luca/Desktop/gta/part-00000")

    val b = matchFound.map(x => (x.firstEntityID, x.secondEntityID))
    val dup1 = b.groupByKey().filter(_._2.size > 1).count()


    println(matchFound.count()*1.0/groundtruth.count())

    val notFound = groundtruth.subtract(matchFound).map(n => (n.firstEntityID, n.secondEntityID)).groupByKey()
    ...
    val dataset1 = CSVWrapper.loadProfiles(pathDataset1)
    ...
    println("Time to generate blocks "+(blocksTime.getTimeInMillis-profilesTime.getTimeInMillis)+" ms")
    println()
