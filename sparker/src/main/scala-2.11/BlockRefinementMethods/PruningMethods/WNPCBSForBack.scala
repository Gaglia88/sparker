...
object WNPCBSForBack {
  ...
  def WNPSenzaArrayFound(profileBlocksFiltered : RDD[ProfileBlocks], blockIndex : Broadcast[scala.collection.Map[Long, (List[Long], List[Long])]],
                         maxID : Int, separatorID : Long, groundtruth : Broadcast[scala.collection.immutable.Set[(Long, Long)]],
                         thresholdType : String = thresholdTypes.AVG) : RDD[(Double, UnweightedEdge)] = {
    profileBlocksFiltered mapPartitions{
      partition =>
          ...
          pb =>
            ...
            for(i <- 0 to numeroVicini-1) {                             //Scorro i vicini che ho trovato
              if(arrayPesi(arrayVicini(i)) >= soglia){                  //Questo vicino ha un peso superiore della soglia
                cont += 1                                               //Incremento il contatore che mi dice quanti vicini ho mantenuto
                if(profileID < arrayVicini(i)) {                      //Il groundtruth è organizzato come (ID dataset1, ID dataset2), quindi devo cercare il profilo con ID minore
                  if(groundtruth.value.contains((profileID, arrayVicini(i)))){         //Se l'id c'è e il suo profilo che matcha è uguale a questo, allora ho trovato il match
                    perfectMatchEdge = UnweightedEdge(profileID, arrayVicini(i)) //Genero l'edge che voglio tenere
                  }
                }
                else{                                                 //Stessa cosa di sopra ma girata
                  if(groundtruth.value.contains((arrayVicini(i), profileID))) {
                    perfectMatchEdge = UnweightedEdge(arrayVicini(i), profileID)
                  }
                }
              }
  ...
  def CalcPCPQ(profileBlocksFiltered : RDD[ProfileBlocks], blockIndex : Broadcast[scala.collection.Map[Long, (List[Long], List[Long])]], maxID : Int, separatorID : Long, groundtruth : Broadcast[scala.collection.Map[Long, Long]]) : RDD[(Double, UnweightedEdge)] = {
    profileBlocksFiltered mapPartitions{
      partition =>
  ...
  def WNPAdv(profileBlocksFiltered : RDD[ProfileBlocks], blockIndex : Broadcast[scala.collection.Map[Long, (List[Long], List[Long])]], maxID : Int, separatorID : Long, WNPType : String = WNPTypes.ANY) : RDD[(Long, Long)] = {

    val nodeWithNeighbors = profileBlocksFiltered.mapPartitions {
      partition =>
        val weights = Array.fill[Int](maxID.toInt + 1) { 0 }
        val neighbours = Array.ofDim[Int](maxID.toInt + 1)
        var neighboursNumbers = 0

        partition map {
          pb =>
            val profileID = pb.profileID
            val blocksInWhichAppears = pb.blocks

            blocksInWhichAppears foreach {
              block =>
                val blockID = block.blockID
                val currentBlockProfiles = blockIndex.value.get(blockID)
                if (currentBlockProfiles.isDefined) {
                  val containedProfiles = {
                    if (separatorID >= 0 && profileID <= separatorID) {
                      currentBlockProfiles.get._2
                    }
                    else {
                      currentBlockProfiles.get._1
                    }
                  }

                  containedProfiles foreach {
                    secondProfileID =>
                      val neighbour = secondProfileID.toInt
                      val currentNeighbourWeight = weights(neighbour)
                      if (currentNeighbourWeight == 0) {
                        neighbours.update(neighboursNumbers, neighbour)
                        weights.update(neighbour, 1)
                        neighboursNumbers = neighboursNumbers + 1
                      }
                      else {
                        weights.update(neighbour, currentNeighbourWeight + 1)
                      }
                  }
                }
            }

            val currentProfileNeighbours = for (i <- 0 to neighboursNumbers - 1) yield {
              val neighbourID = neighbours(i)
              val neighbourWeight = weights(neighbourID)
              weights.update(neighbourID, 0)
              (neighbourID, neighbourWeight)
            }

            val avg = currentProfileNeighbours.map(_._2).sum.toFloat/neighboursNumbers.toFloat
            neighboursNumbers = 0

            (profileID, currentProfileNeighbours, avg)
        }
    }

    val sc = SparkContext.getOrCreate()

    val avgNodeWeights = sc.broadcast(nodeWithNeighbors.map{case (profileID, neighbours, avgWeight) => (profileID, avgWeight)}.collectAsMap())

    val edges = nodeWithNeighbors.flatMap{
      case (nodeID, edges, nodeAVG) =>
        for(
          (neighbourID, neighbourWeight) <- edges
          if (WNPType == WNPTypes.OR && (neighbourWeight >= nodeAVG || neighbourWeight >= avgNodeWeights.value(neighbourID)) && (nodeID < neighbourID)) ||
            (WNPType == WNPTypes.AND && neighbourWeight >= nodeAVG && neighbourWeight  >= avgNodeWeights.value(neighbourID) && nodeID < neighbourID) ||
            (WNPType == WNPTypes.ANY)
        ) yield {
          if(nodeID < neighbourID){
            (nodeID.toLong, neighbourID.toLong)
          }
          else{
            (neighbourID.toLong, nodeID.toLong)
          }
        }
    }
    avgNodeWeights.unpersist()
    edges
  }
