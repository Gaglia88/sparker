...
import BlockBuildingMethods.LSHTwitterOld.Settings
...
object LSHSpark {
    ...
    val tokens = profiles.flatMap {
      profile =>
        profile.attributes.flatMap {
          kv =>
            kv.value.split(BlockingUtils.TokenizerPattern.DEFAULT_SPLITTING).map {
              token =>
                token
            }
        }
  ...
  def clusterSimilarAttributes(profiles: RDD[Profile], numHashes : Int, numBands : Int, separatorID: Long = -1): List[KeysCluster] = {
    val sqlContext = SparkSession.builder.getOrCreate()
    import sqlContext.implicits._
    ...
    val sparseVectorData = LSHSpark.createKeysSparseVector(profiles)
    ...
    //Trasformo il dataframe
    val profileWithBuckets = profileWithHashes.map{
      case Row(id : String, hash : mutable.WrappedArray[org.apache.spark.ml.linalg.Vector]) =>
        /*
        * Il primo map mette tutti gli elementi in un'unica lista, perché prima era un array di array.
        * La funzione grouped trasforma la lista in un elenco di liste in modo da avere il numero di
        * bande richieste.
    ...
    //Converto il dataframe in rdd
    val profileWithBucketsRDD = profileWithBuckets.rdd.map(r => (r.getString(0), r.getAs[Iterable[Int]](1)))

    val clusters = profileWithBucketsRDD.flatMap(x => x._2.map((_, x._1))).groupByKey().filter(_._2.size > 1)
  ...
  /**
    * Dato un RDD di profili lo trasforma in uno sparse vector
    * */
  def createKeysSparseVector(profiles : RDD[Profile], separatorID : Long = -1) : RDD[(String, SparseVector)] = {
    /** Genero tutti i token e gli assegno un id univoco */
    val tokens = profiles.flatMap {
      profile =>
        profile.attributes.flatMap {
