...
object CSVWrapper extends WrapperTrait{
  ...
  /**
    ...
    * */
  override  def loadProfiles(filePath : String, startIDFrom : Long): RDD[Profile] ={
    loadProfiles(filePath, startIDFrom)
  }
  ...
  def loadProfiles(filePath : String, startIDFrom : Long = 0, separator : String = ",", header : Boolean = false,
                   explodeInnerFields:Boolean = false, innerSeparator : String = ",") : RDD[Profile] = {
  ...
  def loadGroundtruth(filePath : String, separator : String = ",", header : Boolean = false) : RDD[MatchingEntities] = {
    val sparkSession = SparkSession.builder().config("header", header).config("sep", separator).getOrCreate()
    val df = sparkSession.read.csv(filePath)
    df.rdd map {
      row =>
