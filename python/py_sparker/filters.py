from objects import BlockClean, BlockDirty, ProfileBlocks
...
class BlockPurging(object):
	...
	def blockPurging(blocks, smoothFactor):
		...
		totalNumberOfComparisonsAndSizePerComparisonLevel = blockComparisonsAndSizesPerComparisonLevel.reduceByKey(lambda x, y : (x[0]+y[0], x[1]+y[1]))
		totalNumberOfComparisonsAndSizePerComparisonLevelSorted = totalNumberOfComparisonsAndSizePerComparisonLevel.sortBy(lambda x: x[0]).collect()
		...
		totalNumberOfComparisonsAndSizePerComparisonLevelSortedAdded = sumPrecedentLevels(totalNumberOfComparisonsAndSizePerComparisonLevelSorted)
		...
		maximumNumberOfComparisonAllowed = calcMaxComparisonNumber(totalNumberOfComparisonsAndSizePerComparisonLevelSortedAdded, smoothFactor)
